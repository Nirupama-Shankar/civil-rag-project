...
async def get_graph_concepts():
    ...
    from .graph_builder import get_all_concepts
    try:
...
@app.get("/graph/relationships/{concept}")
async def get_concept_relationships(concept: str, depth: int = 2):
    """Get relationships for a concept"""
    from .graph_builder import query_relationships
    try:
        return query_relationships(concept, depth)
    except Exception as e:
        return {"concept": concept, "related": [], "error": str(e)}

@app.get("/graph/documents/{concept}")
async def get_documents_for_concept(concept: str):
    """Get documents containing a concept"""
    from .graph_builder import get_documents_for_concept
    try:
        docs = get_documents_for_concept(concept)
        return {"concept": concept, "documents": docs}
    except Exception as e:
        return {"concept": concept, "documents": [], "error": str(e)}

async def get_documents_for_concept(concept: str):
    """Get documents containing a concept"""
    from .graph_builder import get_documents_for_concept
    return {"documents": get_documents_for_concept(concept)}

@app.get("/graph/visualize/{concept}")
async def visualize_concept(concept: str):
